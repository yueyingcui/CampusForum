...
public class AppMemberListS extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("-----sip/AppMemberListS.java | SIP/appmemberlist.jsp-----");
		...
		if(submit_accept != null)
		{
			try{
				...
				// querry1: insert SIPMEMBER
				// get primary key +1
				Statement stmt1 = conn.createStatement();
				String query1 = "SELECT SIPMEMBERID FROM SIPMEMBER";
				System.out.println(query1);
				ResultSet rs1 = stmt1.executeQuery(query1);
				int sipmemberid_db = 0;
				while(rs1.next())
				{
					sipmemberid_db = rs1.getInt(1);
				}
				sipmemberid_db = sipmemberid_db + 1;
				// insert (querry2)
				String query2 = "INSERT INTO SIPMEMBER (SIPMEMBERID, SIPID, MEMBERID) VALUES ('" + sipmemberid_db + "', '" + sipid_choose + "', '" + senderid_choose + "')";
				System.out.println(query2);
				stmt1.executeUpdate(query2);
				// querry3: delete SIPREQUESTMEMBER
				Statement stmt2 = conn.createStatement();
				String query3 = "DELETE FROM SIPREQUESTMEMBER WHERE (SENDERID = '" + senderid_choose + "' AND SIPID = '" + sipid_choose + "')";
				System.out.println(query3);
				stmt2.executeUpdate(query3);
				
				// redirect webpage
					System.out.println("Accept sip member request successfully!");
		...
		if(submit_deny != null)
		{
			try{
				...
				HttpSession session = request.getSession();
				hostid = (Integer)session.getAttribute("hostid");
				// querry1: delete SIPREQUESTMEMBER
				Statement stmt1 = conn.createStatement();
				String query1 = "DELETE FROM SIPREQUESTMEMBER WHERE (SENDERID = '" + senderid_choose + "' AND SIPID = '" + sipid_choose + "')";
				System.out.println(query1);
				stmt1.executeUpdate(query1);
				
				// redirect webpage
					System.out.println("Deny sip member request successfully!");
					response.sendRedirect("jsp/SIP/sip.jsp");
