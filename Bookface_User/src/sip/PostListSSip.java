...
public class PostListSSip extends HttpServlet {
...
System.out.println("-----sip/PostListSSip.java | SIP/postlist.jsp-----");
		...
		if(submit_createcomment != null)
		{
			...
			String postid1 = request.getParameter("sippostid_choose");
			...
			request.getRequestDispatcher("jsp/SIP/createcomment.jsp").forward(request,response);		
		}
		...
		if(submit_commentlist != null)
		{
			try{
				...
				// select SIPCOMMENT, save selecct result
				// querry1: ownerid, content, datetime
				Statement stmt1 = conn.createStatement();
				String query1 = "SELECT OWNERID, CONTENT, DATETIME FROM SIPCOMMENT WHERE SIPPOSTID = '" + postid + "'";
				...
				request.getRequestDispatcher("jsp/SIP/commentlist.jsp").forward(request,response);
		...
		if(submit_modifypost != null)
		{
			...
			request.getRequestDispatcher("jsp/SIP/modifypost.jsp").forward(request,response);
		}
		...
		if(submit_deletepost != null)
		{
			try{
				// JSP information
				response.setContentType("text/html; charset=gbk"); 
				String postid_choose1 = request.getParameter("sippostid_choose");
				...
				//	DB2 information
				// querry1: delete SIPCOMMENT
				Statement stmt1 = conn.createStatement();
				String query1 = "DELETE FROM SIPCOMMENT WHERE (SIPPOSTID = '" + postid_choose + "')";
				System.out.println(query1);
				stmt1.executeUpdate(query1);
				// querry2: delete SIPPOST
				Statement stmt2 = conn.createStatement();
				String query2 = "DELETE FROM SIPPOST WHERE (SIPPOSTID = '" + postid_choose + "')";
				System.out.println(query2);
