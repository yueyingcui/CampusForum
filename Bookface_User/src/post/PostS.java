...
public class PostS extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("-----post/PostS.java | Post/post.jsp-----");
		
		// choose "submit" 
		String submit_wall = request.getParameter("wall");
		String submit_home = request.getParameter("home");
		...
		if(submit_wall != null)
		{
			try{
				...
				String page_ownerid = Integer.toString(hostid);
				
				// set Attibute
				request.setAttribute("page_ownerid", page_ownerid);
				System.out.println("page_ownerid: " + page_ownerid);	
				
				// redirect webpage
				System.out.println("Go to user's Wall!");
				request.getRequestDispatcher("jsp/Post/wall.jsp").forward(request,response);
		...
		if(submit_home != null)
		{
			try{
				...
				HttpSession session = request.getSession();
				hostid = (Integer)session.getAttribute("hostid");
				// select FRIEND, save select result
				// querry0: friendid
				Statement stmt0 = conn.createStatement();
				String query0 = "SELECT OWNERFRIENDID FROM FRIEND WHERE OWNERID = '" + hostid + "'";
				System.out.println(query0);
				ResultSet rs0 = stmt0.executeQuery(query0);
				while(rs0.next())
				{
					// assign value
					postpage_ownerid_db = rs0.getInt(1);
					
					// select POSTPAGE, save select result
					// querry1: postpageid
					Statement stmt1 = conn.createStatement();
					String query1 = "SELECT POSTPAGEID FROM POSTPAGE WHERE OWNERID = '" + postpage_ownerid_db + "'";
