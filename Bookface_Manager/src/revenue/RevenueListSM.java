...
public class RevenueListSM extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// choose "submit" 
		String submit_listbyitem = request.getParameter("listbyitem");
		String submit_listbytype = request.getParameter("listbytype");
		String submit_listbycustomer = request.getParameter("listbycustomer");
		
		if(submit_listbyitem != null)
		{
			try{
				// JSP information
				...
				request.getRequestDispatcher("jsp/Revenue/rlitemlistM.jsp").forward(request,response);
		...
		if(submit_listbytype != null)
		{
			try{
				...
				// querry1: type
				Statement stmt1 = conn.createStatement();
				String query1 = "SELECT ADVERTISEMENTID, TYPE, UNITPRICE FROM ADVERTISEMENT";
				System.out.println(query1);
				ResultSet rs1 = stmt1.executeQuery(query1);
				while(rs1.next())
				{
					...
					if(type_exist.equals("false"))
					{
						...
						unitprice_list.add("" + unitprice_db);
						System.out.println("itemname:" + type_db);
					}						
				}
				...
				request.setAttribute("sum_revenue_list", sum_revenue_list);
				request.setAttribute("advertisementid_list", advertisementid_list);
				request.setAttribute("itemname_list", type_list);
				System.out.println("advertisementid_list.size()=" + advertisementid_list.size());
				System.out.println("itemname_list.size()=" + type_list.size());
				// other characteristics!!!
		...
		if(submit_listbycustomer != null)
		{
			try{
				...
				request.getRequestDispatcher("jsp/Revenue/rlcustomerlistM.jsp").forward(request,response);
